...
export const estimateCostSchema = z.object({
  project_type: z.enum([
    ...
    "kitchen_remodel", "bathroom_remodel", "room_addition",
    "deck_patio", "solar_panels", "electrical_panel",
    "fence_retaining_wall", "hvac_replacement", "roofing", "garage_new",
  ]).describe("Type of home improvement project"),
  size_sqft: z.number().optional().describe("Project size in sq ft (for ADU, addition, deck, roofing, garage)"),
  scope: z.enum(["minor", "mid", "major", "luxury"]).optional().describe("Scope level for remodels (kitchen/bathroom)"),
  system_size_kw: z.number().optional().describe("Solar system size in kilowatts"),
  linear_feet: z.number().optional().describe("Length in linear feet (for fence/retaining wall)"),
  coastal_zone: z.boolean().optional().default(false).describe("Property is in a coastal overlay zone"),
  historic_district: z.boolean().optional().default(false).describe("Property is in a historic district or structure is 45+ years old"),
  hillside: z.boolean().optional().default(false).describe("Property is on a steep hillside (requires geo report)"),
});
...
export function handleEstimateCost(input: EstimateCostInput): string {
  const data = PROJECT_DATA[input.project_type];
  if (!data) {
    return JSON.stringify({ error: `Unknown project type: ${input.project_type}` });
  }
  ...
  // Use midpoint of construction cost as valuation for permit fee calc
  ...
  // School impact fees (ADU-specific)
  let schoolImpact = 0;
  ...
  // Water/sewer (ADU-specific)
  let waterSewer = 0;
  if (data.isAdu) {
    waterSewer = 4101; // ~$1,524 water + ~$2,577 sewer at 0.5 EDU
  }
  ...
  // Major kitchen/bathroom remodels may need architect + engineer
